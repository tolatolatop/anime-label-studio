...
def extract_subtitles(mkv_path, output_dir):
    ...
    for track in subtitle_tracks:
        ...
        lang = 'und'  # 默认未知语言
        try:
            info = subprocess.check_output(cmd_lang, stderr=subprocess.STDOUT, text=True)
            for line in info.split('\n'):
                line = line.strip()
        ...
        # 使用mkvextract提取字幕
        cmd_extract = ['mkvextract', 'tracks', str(mkv_path), f"{track}:{output_path}"]
        try:
...
if __name__ == "__main__":
    ...
    # 检查mkvextract是否可用
    try:
        subprocess.run(['mkvextract', '--version'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except:
